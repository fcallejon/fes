    ...
    type BestFields =
        ...
        static member ToJson bestFields =
            jobj [ "query" .= bestFields.Query
                   ...
                   "auto_generate_synonyms_phrase_query"
                   .=? bestFields.AutoGenerateSynonymsPhraseQuery
                   "fuzzy_transportations"
                   .=? bestFields.FuzzyTransportations
    ...
    type BoolPrefix =
        ...
        static member ToJson boolPrefix =
            jobj [ "query" .= boolPrefix.Query
                   ...
                   "fuzzy_transportations"
                   .=? boolPrefix.FuzzyTransportations
                   "zero_terms_query" .=? boolPrefix.ZeroTermsQuery
                   "auto_generate_synonyms_phrase_query"
                   .=? boolPrefix.AutoGenerateSynonymsPhraseQuery ]
...
type CombinedFields =
    ...
    static member ToJson queryString =
        jobj [ "query" .= queryString.Query
               ...
               "minimum_should_match"
               .=? queryString.MinimumShouldMatch
...
type SimpleQueryString =
    ...
    static member ToJson simpleQueryString =
        let flags =
            let join =
                (Array.map SimpleQueryStringLimitOperator.ToString)
                >> (fun x -> String.Join("|", x))

            simpleQueryString.Flags |> Option.map join

        jobj [ "query" .= simpleQueryString.Query
               "fields" .= simpleQueryString.Fields
               "default_operator"
               .=? simpleQueryString.DefaultOperator
               "analyze_wildcard"
               .=? simpleQueryString.AnalyzeWildcard
               "analyzer" .=? simpleQueryString.Analyzer
               "auto_generate_synonyms_phrase_query"
               .=? simpleQueryString.AutoGenerateSynonymsPhraseQuery
               "flags" .=? flags
               "fuzzy_max_expansions"
               .=? simpleQueryString.FuzzyMaxExpansions
               "fuzzy_prefix_length"
               .=? simpleQueryString.FuzzyPrefixLength
               "fuzzy_transpositions"
               .=? simpleQueryString.FuzzyTransportations
               "lenient" .=? simpleQueryString.Lenient
               "minimum_should_match"
               .=? simpleQueryString.MinimumShouldMatch
               "quote_field_suffix"
               .=? simpleQueryString.QuoteFieldSuffix ]
...
with
    static member ToJson query =
        match query with
        | AllOfInterval q -> toJson q
        | AnyOfInterval q -> toJson q
        | MatchQueries q -> toJson q
        | CombinedFields q -> toJson q
        | QueryString q -> toJson q
        | SimpleQueryString q -> toJson q