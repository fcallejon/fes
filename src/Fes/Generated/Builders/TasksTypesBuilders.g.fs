...
module TasksTypesBuilders =
    ...
    type NodeTasksBuilder() =
        member _.Yield(_: unit) : NodeTasks =
            {
                Name = None
        ...
        member _.Roles(state: NodeTasks, value: string list) =
            { state with Roles = Some value }

        [<CustomOperation("attributes")>]
        member _.Attributes(state: NodeTasks, value: Map<string, string>) =
    ...
    type TaskInfoBuilder() =
        member _.Yield(_: unit) : TaskInfo =
            {
        ...
        member _.Action(state: TaskInfo, value: string) =
            { state with Action = value }

        [<CustomOperation("cancelled")>]
        member _.Cancelled(state: TaskInfo, value: bool) =
            { state with Cancelled = Some value }

        [<CustomOperation("cancellable")>]
        member _.Cancellable(state: TaskInfo, value: bool) =
            { state with Cancellable = value }

        [<CustomOperation("description")>]
        member _.Description(state: TaskInfo, value: string) =
            { state with Description = Some value }

        [<CustomOperation("headers")>]
        member _.Headers(state: TaskInfo, value: Map<string, string>) =
        ...
        member _.Status(state: TaskInfo, value: System.Text.Json.JsonElement) =
            { state with Status = Some value }

        [<CustomOperation("type'")>]
        member _.Type(state: TaskInfo, value: string) =
    ...
    type TaskListResponseBaseBuilder() =
        member _.Yield(_: unit) : TaskListResponseBase =
            {
                NodeFailures = None
                TaskFailures = None
                Nodes = None
        ...
        member _.Nodes(state: TaskListResponseBase, value: Map<string, TasksTypes.NodeTasks>) =
            { state with Nodes = Some value }

        [<CustomOperation("tasks")>]
        member _.Tasks(state: TaskListResponseBase, value: TasksTypes.TaskInfos) =
