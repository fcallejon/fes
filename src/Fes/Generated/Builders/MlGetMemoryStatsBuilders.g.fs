...
module MlGetMemoryStatsBuilders =
    ...
    type JvmStatsBuilder() =
        ...
        member _.HeapMax(state: JvmStats, value: Types.ByteSize) =
            { state with HeapMax = Some value }

        [<CustomOperation("heapMaxInBytes")>]
        member _.HeapMaxInBytes(state: JvmStats, value: Types.Integer) =
            { state with HeapMaxInBytes = value }

        [<CustomOperation("javaInference")>]
        member _.JavaInference(state: JvmStats, value: Types.ByteSize) =
            { state with JavaInference = Some value }

        [<CustomOperation("javaInferenceInBytes")>]
        member _.JavaInferenceInBytes(state: JvmStats, value: Types.Integer) =
            { state with JavaInferenceInBytes = value }

        [<CustomOperation("javaInferenceMax")>]
        member _.JavaInferenceMax(state: JvmStats, value: Types.ByteSize) =
            { state with JavaInferenceMax = Some value }

        [<CustomOperation("javaInferenceMaxInBytes")>]
        member _.JavaInferenceMaxInBytes(state: JvmStats, value: Types.Integer) =
    ...
    type MemMlStatsBuilder() =
        ...
        member _.AnomalyDetectors(state: MemMlStats, value: Types.ByteSize) =
            { state with AnomalyDetectors = Some value }

        [<CustomOperation("anomalyDetectorsInBytes")>]
        member _.AnomalyDetectorsInBytes(state: MemMlStats, value: Types.Integer) =
            { state with AnomalyDetectorsInBytes = value }

        [<CustomOperation("dataFrameAnalytics")>]
        member _.DataFrameAnalytics(state: MemMlStats, value: Types.ByteSize) =
            { state with DataFrameAnalytics = Some value }

        [<CustomOperation("dataFrameAnalyticsInBytes")>]
        member _.DataFrameAnalyticsInBytes(state: MemMlStats, value: Types.Integer) =
            { state with DataFrameAnalyticsInBytes = value }

        [<CustomOperation("max")>]
        member _.Max(state: MemMlStats, value: Types.ByteSize) =
            { state with Max = Some value }

        [<CustomOperation("maxInBytes")>]
        member _.MaxInBytes(state: MemMlStats, value: Types.Integer) =
            { state with MaxInBytes = value }

        [<CustomOperation("nativeCodeOverhead")>]
        member _.NativeCodeOverhead(state: MemMlStats, value: Types.ByteSize) =
            { state with NativeCodeOverhead = Some value }

        [<CustomOperation("nativeCodeOverheadInBytes")>]
        member _.NativeCodeOverheadInBytes(state: MemMlStats, value: Types.Integer) =
            { state with NativeCodeOverheadInBytes = value }

        [<CustomOperation("nativeInference")>]
        member _.NativeInference(state: MemMlStats, value: Types.ByteSize) =
            { state with NativeInference = Some value }

        [<CustomOperation("nativeInferenceInBytes")>]
        member _.NativeInferenceInBytes(state: MemMlStats, value: Types.Integer) =
    ...
    type MemStatsBuilder() =
        ...
        member _.AdjustedTotal(state: MemStats, value: Types.ByteSize) =
            { state with AdjustedTotal = Some value }

        [<CustomOperation("adjustedTotalInBytes")>]
        member _.AdjustedTotalInBytes(state: MemStats, value: Types.Integer) =
            { state with AdjustedTotalInBytes = value }

        [<CustomOperation("total")>]
        member _.Total(state: MemStats, value: Types.ByteSize) =
            { state with Total = Some value }

        [<CustomOperation("totalInBytes")>]
        member _.TotalInBytes(state: MemStats, value: Types.Integer) =
